...
System.out.println(" mac:"+ob.getMac(ipad));
	...
	String mac = ""+ob.getMac(ipad);
	
	String dname = ""+name.get(i).toString();
	
	
			Class.forName("com.mysql.jdbc.Driver");  
Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/wifi","root","");  

Statement st = con.createStatement();
			
			ResultSet r= st.executeQuery("select mac from student  where   mac ='"+mac+"'   ");
...
st.executeUpdate("insert into devices values('"+ipad+"','"+dname+"','"+mac+"','"+datetime+"','"+status+"' ) ");
